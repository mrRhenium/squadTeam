...
// 1 rank holder to finalist according to winning matches
...
if (setting[0].tournamentRankingType.byWinningMatches) {
  ...
  finalPageBtn.addEventListener("click", () => {
    ...
    if (
      ...
      Data.matchSquad.push({
        TeamName1: `${Data.playersPositionAccordingToWinMatches[1].name}`,
        TeamName2: `${Data.playersPositionAccordingToWinMatches[2].name}`,
      ...
      Data.playersPositionAccordingToWinMatches.map((item, index) => {
        if (index > 2) {
          playerRank.push({
            name: item.name,
            rank: index,
      ...
      // put the new touranment details in tournaments history json file in local storage
  ...
  console.log("Ranking is decided by total points Matches");
}
// ends here
